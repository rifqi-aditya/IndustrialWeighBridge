 ...
 *
 * State machine transitions (as per HLD): Idle -> WeighingIn (when starting weigh-in) WeighingIn ->
 * Idle (when weigh-in captured successfully, transaction created) Idle -> WeighingOut (when
 * selecting active transaction for weigh-out) WeighingOut -> Completed (when weigh-out captured and
 * transaction closed) Completed -> Idle (after displaying results/printing ticket)
 *
...
sealed class WeighingState {
    ...
    /**
     * System is in weigh-in mode (first weighing). Waiting for weight to stabilize and be captured.
     */
