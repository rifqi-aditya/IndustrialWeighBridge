...
fun TransactionHistoryScreen() {
        ...
        val filteredTransactions =
                uiState.allTransactions.filter { txn ->
                        val matchesSearch =
                                searchQuery.isEmpty() ||
                                        txn.ticket_number.contains(
                                                searchQuery,
                                                ignoreCase = true
                                        ) ||
                                        (txn.plate_number?.contains(
                                                searchQuery,
                                                ignoreCase = true
                                        ) == true) ||
                                        (txn.driver_name?.contains(
                                                searchQuery,
                                                ignoreCase = true
                                        ) == true) ||
                                        (txn.product_name?.contains(
                                                searchQuery,
                                                ignoreCase = true
                                        ) == true)
                                ...
                                LazyColumn(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                                        items(filteredTransactions) { txn ->
                                                TransactionCard(
                                                        transaction = txn,
                                                        onPrint = { transaction ->
                                                                com.rifqi.industrialweighbridge
                                                                        .presentation.print
                                                                        .printTransaction(
                                                                                transaction,
                                                                                settingsRepository
        ...
        Card(
                ...
                Column(modifier = Modifier.padding(16.dp)) {
                        ...
                        Row(
                                ...
                                Row(verticalAlignment = Alignment.CenterVertically) {
                                        Icon(
                                                ...
                                                tint =
                                                        if (isOpen) Color(0xFFFF9800)
                                                        else Color(0xFF4CAF50)
                                        )
                                        Spacer(modifier = Modifier.width(4.dp))
                                        Text(
                                                text = if (isOpen) "TERBUKA" else "SELESAI",
                        ...
                        Row(
                                ...
                                Text(
                                        text =
                                                "Masuk: ${DateTimeUtils.formatForDisplay(transaction.weigh_in_timestamp)}",
                                        style = MaterialTheme.typography.labelSmall,
                                        color = MaterialTheme.colorScheme.onSurfaceVariant
                                )
                                transaction.weigh_out_timestamp?.let {
                                        Text(
                                                text =
                                                        "Keluar: ${DateTimeUtils.formatForDisplay(it)}",
                                                style = MaterialTheme.typography.labelSmall,
                                                color = MaterialTheme.colorScheme.onSurfaceVariant
                                        )
                                }
